...
export default function EnhancedChartVisualization({ query }: ChartVisualizationProps) {
  ...
  useEffect(() => {
    ...
    if (!query?.visualization?.data) {
      if (selectedDay) {
        // For date-based queries, always load depth profile from that day's CSV
        loadDayChartData(selectedDay, chartType)
      } else if (selectedProfile) {
        // Fallback: use profile-based endpoints
  ...
  const loadDayChartData = async (day: string, type: string) => {
    setLoading(true)
    try {
      // CSV-backed depth profile for the selected date; profile index is 0 for now
      const response = await visualizationAPI.getDayDepthProfile(day, 0)
      const data = response.data || []

      // Shape is already { depth, temperature?, salinity? }
      if (type === 'temperature' || type === 'salinity' || type === 'ts-diagram') {
        setChartData(data)
      } else {
        setChartData(data)
      }
    } catch (error) {
      console.error('Failed to load day-based chart data:', error)
      setChartData([])
    } finally {
      setLoading(false)
    }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      if (selectedDay) {
        // Export CSV for the selected day (CSV generated from NetCDF)
        blob = await visualizationAPI.exportDayCSV(selectedDay)
      } else {
        if (!selectedProfile) return
        blob = await visualizationAPI.exportProfileCSV(selectedProfile)
      }
      const url = window.URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = selectedDay ? `argo_${selectedDay}.csv` : `argo_profile_${selectedProfile}.csv`

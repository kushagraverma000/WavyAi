...
export default function DataTablePanel({ query }: DataTablePanelProps) {
  ...
  // Load data based on query (fall back to base data when query has no table)
  useEffect(() => {
    // Prefer data bundled in the query response if available
    if (query?.data_table) {
      ...
      if (dataType === 'floats' && query.data_table.floats) {
    ...
    // Otherwise, always load base data (profiles/floats) so the table is never empty
    loadData()
  }, [query, dataType])
